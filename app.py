    ...
    if role == 'admin':
            ...
            with c_r:
                ...
                with st.container(border=True):
                    ...
                    if st.button("🎁 确认赏赐", type="primary", key="btn_rew"):
                        ...
                        st.balloons()
                        st.success(f"已赏赐 {target_r} {amt_r} YVP")
        ...
        for i, r in my.iterrows():
            with st.container(border=True):
                ...
                with st.expander("👁️ 详情"): st.write(r.get('description', '无'))
                if st.button("✅ 交付", key=f"dev_{r['id']}", type="primary"):
